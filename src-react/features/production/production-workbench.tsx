...
export function ProductionWorkbench({ api, project, pollIntervalMs = 3_000, onOpenAgent, renderProductionAgent }: ProductionWorkbenchProps) {
  ...
  async function switchScript(nextScriptId: number) {
    ...
    if (hasRunningTask && !window.confirm("当前剧本仍有生成任务，切换后任务会继续在后台执行。确定切换吗？")) return;
    setSwitchingScript(true);
    setError("");
    try {
      if (scriptId != null) await api.saveFlowData(project.id, scriptId, { ...flowData, storyboard: storyboards });
  ...
  function openTab(nextTab: "generation" | "flow" | "editor") {
    if (nextTab === "editor") setEditorActivated(true);
    setTab(nextTab);
  }
  ...
  async function insertStoryboard(referenceId: number, placement: "before" | "after") {
    ...
    try {
      ...
      const nextFlow = { ...flowData, storyboard: ordered };
      setFlowData(nextFlow);
      await api.saveFlowData(project.id, scriptId, nextFlow);
    } catch (cause) {
    ...
    <main className="min-h-screen bg-[#090b10] p-5 text-slate-100 lg:p-8">
      <header className="mx-auto mb-6 flex max-w-[1500px] flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">从合同、分镜图、视频生成到 WebAV 剪辑均可在此恢复和继续。</p>
        </div>
      ...
      <div className={tab === "flow" ? "mx-auto max-w-none" : "mx-auto max-w-[1500px]"}>
        <nav aria-label="生产工作台视图" className="mb-5 flex flex-wrap gap-2">
          {(
            [
              ["generation", "生成工作台"],
              ["flow", "产线图"],
            ...
            <button
              ...
              type="button"
              onClick={() => openTab(id)}
              className={`rounded-lg px-3 py-2 text-xs ${tab === id ? "bg-blue-600" : "border border-slate-800 bg-slate-950"}`}>
              {label}
            </button>
          ))}
          ...
          <button
            type="button"
            aria-label="刷新生产数据"
            disabled={scriptId == null || loading}
            onClick={() => scriptId != null && void loadProductionData(scriptId)}
            className="ml-auto rounded-lg border border-slate-800 bg-slate-950 p-2 text-slate-400 hover:text-slate-100 disabled:opacity-50">
              ...
              <div className="mb-3 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    onClick={() =>
                      void api
                        .previewStoryboards(storyboards.filter((item) => item.src).map((item) => item.id))
                        .then(setStoryboardPreview)
                        .catch((cause) => setError(messageOf(cause)))
                    }
                    className="rounded-md border border-slate-700 px-2 py-1 text-xs">
                    预览 / 下载
                  </button>
                  <span className="py-1 text-xs text-slate-500">{storyboards.length} 个镜头</span>
                </div>
              </div>
              {storyboardPreview ? (
                <div className="mb-3 rounded-xl border border-slate-800 bg-slate-950 p-3">
                  <img src={storyboardPreview} alt="分镜合并预览" className="w-full rounded-lg" />
                  <a href={storyboardPreview} download="storyboard-preview.jpg" className="mt-2 inline-flex items-center gap-1 text-xs text-blue-300">
                ...
                {storyboards.map((storyboard, position) => (
                  <article
                    ...
                    <div className="space-y-3 p-4">
                      ...
                      <textarea
                        aria-label={`分镜画面提示词 ${storyboard.id}`}
                        value={storyboard.prompt}
                        onChange={(event) =>
                          setStoryboards((current) =>
                            current.map((item) => (item.id === storyboard.id ? { ...item, prompt: event.target.value } : item)),
                          )
                        }
                        onBlur={() =>
                          void api.editStoryboard(storyboard.id, storyboard.prompt, storyboard.videoDesc).catch((cause) => setError(messageOf(cause)))
                        }
                        className="h-16 w-full rounded-lg border border-slate-800 bg-slate-900 p-2 text-sm font-medium"
                        placeholder="未填写画面描述"
                      />
                      <textarea
                        aria-label={`分镜视频描述 ${storyboard.id}`}
                        value={storyboard.videoDesc}
                        onChange={(event) =>
                          setStoryboards((current) =>
                            current.map((item) => (item.id === storyboard.id ? { ...item, videoDesc: event.target.value } : item)),
                          )
                        }
                        onBlur={() =>
                          void api.editStoryboard(storyboard.id, storyboard.prompt, storyboard.videoDesc).catch((cause) => setError(messageOf(cause)))
                        }
                        className="h-14 w-full rounded-lg border border-slate-800 bg-slate-900 p-2 text-xs text-slate-400"
                        placeholder="视频运动描述"
                      />
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <button
                          ...
                          onClick={() => void insertStoryboard(storyboard.id, "before")}
                          className="rounded-lg border border-slate-800 px-2 py-1.5 text-xs text-slate-400">
                        ...
                        <button
                          ...
                          onClick={() => void insertStoryboard(storyboard.id, "after")}
                          className="rounded-lg border border-slate-800 px-2 py-1.5 text-xs text-slate-400">
                          <Plus className="mr-1 inline size-3" />
                          后插分镜
                        </button>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <button
                          ...
                          className="rounded-lg border border-slate-700 px-2 py-2 text-xs">
                          {storyboard.state === "running"
                            ? "处理中"
                            : storyboard.state === "completed" || storyboard.state === "failed"
                              ? "重新生成分镜图"
                              : "生成分镜图"}
                        </button>
                        <button
                          type="button"
                          onClick={() => setEditingStoryboard(storyboard)}
                          className="rounded-lg border border-slate-700 px-2 py-2 text-xs">
                        ...
                        <button
                          ...
                          onClick={() =>
                            void api
                              .deleteStoryboards(project.id, [storyboard.id])
                              .then(() => setStoryboards((current) => current.filter((item) => item.id !== storyboard.id)))
                              .catch((cause) => setError(messageOf(cause)))
                          }
                          className="rounded-lg border border-red-900 px-2 py-2 text-xs text-red-300">
            ...
            <section aria-labelledby="video-title" className="min-w-0">
              <div className="mb-3 flex items-center justify-between">
                ...
                <button
                  ...
                  className="flex items-center gap-1 rounded-md border border-slate-700 px-2 py-1 text-xs">
              ...
              <div className="space-y-3">
                {tracks.map((track, position) => (
                  <article
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-slate-500">视频轨道 {position + 1}</span>
                      <div className="flex items-center gap-2">
                        <StatusBadge state={track.state} />
                        <button type="button" aria-label={`删除轨道 ${track.id}`} onClick={() => void deleteTrack(track.id)}>
                    ...
                    <div className="grid grid-cols-[1fr_80px] gap-2">
                      <textarea
                        aria-label={`轨道提示词 ${track.id}`}
                        value={track.prompt}
                        onChange={(event) =>
                          setTracks((current) => current.map((item) => (item.id === track.id ? { ...item, prompt: event.target.value } : item)))
                        }
                        onBlur={() => void api.updateTrackPrompt(track.id, track.prompt).catch((cause) => setError(messageOf(cause)))}
                        className="h-20 rounded-lg border border-slate-800 bg-slate-900 p-2 text-xs"
                      />
                      <input
                        aria-label={`轨道时长 ${track.id}`}
                        type="number"
                        min="1"
                        max="30"
                        ...
                        onChange={(event) =>
                          setTracks((current) =>
                            current.map((item) => (item.id === track.id ? { ...item, duration: Number(event.target.value) } : item)),
                          )
                        }
                        onBlur={() => void api.updateTrackDuration(track.id, track.duration).catch((cause) => setError(messageOf(cause)))}
                        className="h-9 rounded-lg border border-slate-800 bg-slate-900 px-2 text-xs"
                      />
                    </div>
                    <div className="flex gap-2 overflow-x-auto">
                      {track.medias.map((media, index) => (
                        <label
                          ...
                          className={`relative size-16 shrink-0 cursor-pointer overflow-hidden rounded-lg border ${media.selected !== false ? "border-blue-500" : "border-slate-800 opacity-50"}`}>
                          <input
                            aria-label={`使用参考素材 ${media.id ?? index}`}
                            type="checkbox"
                            checked={media.selected !== false}
                            onChange={(event) =>
                              setTracks((current) =>
                                current.map((item) =>
                                  item.id === track.id
                                    ? {
                                        ...item,
                                        medias: item.medias.map((entry, entryIndex) =>
                          ...
                          />
                          {media.fileType === "image" && media.src ? (
                    ...
                    {track.videoList.map((video) => (
                      <div
                        key={video.id}
                        className={`rounded-lg border p-3 ${track.selectVideoId === video.id ? "border-emerald-500" : "border-slate-800"}`}>
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-slate-500">视频 #{video.id}</span>
                        ...
                        {video.src && video.state === "completed" ? (
                          <>
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="mt-2 inline-flex items-center gap-1 text-xs text-blue-300">
                              <Download className="size-3" />
                              下载视频
                            </a>
                          </>
                        ...
                        <div className="mt-2 flex gap-2">
                          <button
                            ...
                            onClick={() =>
                              void api
                                .selectVideo(track.id, video.id)
                                .then(() =>
                                  setTracks((current) => current.map((item) => (item.id === track.id ? { ...item, selectVideoId: video.id } : item))),
                                )
                                .catch((cause) => setError(messageOf(cause)))
                            }
                            className="flex-1 rounded border border-slate-700 px-2 py-1 text-xs">
                          ...
                          <button
                                  ...
                                  setTracks((current) =>
                                    current.map((item) =>
                                      item.id === track.id
                                        ? {
                                            ...item,
                                            videoList: item.videoList.filter((entry) => entry.id !== video.id),
                                            selectVideoId: item.selectVideoId === video.id ? null : item.selectVideoId,
                                          }
                                        : item,
                                    ),
                                  ),
                            ...
                            className="rounded border border-red-900 px-2 text-red-300">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        type="button"
                        aria-label="生成轨道提示词"
                        onClick={() => void generatePrompt(track.id)}
                        disabled={track.state === "running"}
                        className="rounded-lg border border-violet-700 px-3 py-2 text-xs">
                      ...
                      <button
                        ...
                        className="rounded-lg bg-blue-600 px-3 py-2 text-xs disabled:bg-slate-800">
        ...
        {!loading && scripts.length && editorActivated ? (
          <div className={tab === "editor" ? "space-y-3" : "hidden"} aria-hidden={tab === "editor" ? undefined : true}>
            {mediaError ? (
              <div
                role="alert"
            ...
            <WebAvVideoEditor key={`${project.id}:${scriptId}`} clips={completedVideos} mediaLibrary={mediaLibrary.map(toEditorMedia)} />
          </div>
        ) : null}
      </div>
      {editingStoryboard && scriptId != null ? (
        <ImageFlowEditor
          api={api}
          projectId={project.id}
          scriptId={scriptId}
          ...
          onSaved={(url, flowId) =>
            setStoryboards((current) =>
              current.map((item) => (item.id === editingStoryboard.id ? { ...item, src: url, flowId, state: "completed" } : item)),
            )
          }
        />
      ) : null}
