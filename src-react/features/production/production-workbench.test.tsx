...
function createApi(scriptItems?: ScriptSummary[]): ProductionApi {
  ...
  return {
    ...
    startMarbleWorld: vi.fn(async () => { throw new Error("未在该测试中使用"); }),
    refreshMarbleWorld: vi.fn(async () => { throw new Error("未在该测试中使用"); }),
    getWorldRegistration: vi.fn(async () => { throw new Error("未在该测试中使用"); }),
    generateWorldRegistration: vi.fn(async () => { throw new Error("未在该测试中使用"); }),
    saveWorldRegistration: vi.fn(async () => { throw new Error("未在该测试中使用"); }),
