...
describe("production flow contract", () => {
  ...
  it("lays out the main chain from measured widths with a fixed 80px gap", () => {
    ...
    expect(productionAutoLayout({ nodeSizes })).toEqual({
      ...
      previs: { x: 3_390, y: 0 },
      videoTracks: { x: 3_990, y: 0 },
      timeline: { x: 4_570, y: 0 },
      finalOutput: { x: 5_150, y: 0 },
      assets: { x: 480, y: 597 },
  ...
  it("falls back to 150x50 only for missing or invalid measurements", () => {
    ...
    expect(layout).toEqual({
      ...
      previs: { x: 1_400, y: 0 },
      videoTracks: { x: 1_630, y: 0 },
      timeline: { x: 1_860, y: 0 },
      finalOutput: { x: 2_090, y: 0 },
      assets: { x: 230, y: 380 },
  ...
  it("moves the colliding main-chain node and every following node to the right", () => {
    ...
    expect(first).toEqual({
      ...
      previs: { x: 1_300, y: 0 },
      videoTracks: { x: 1_480, y: 0 },
      timeline: { x: 1_660, y: 0 },
      finalOutput: { x: 1_840, y: 0 },
      assets: { x: 180, y: 180 },
