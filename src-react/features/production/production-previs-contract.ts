...
function toBlender(value: [number, number, number]): PrevisVector3 {
...
export function createProductionPrevisContract(
  flow: ProductionFlowData,
  projectId: number,
  scriptId: number,
  storyboardId: number,
  ratio: ProductionVideoRatio = "16:9",
): ProductionPrevisShotContract {
  ...
  const moving =
    /走|跑|推进|拉近|跟随|移动|walk|run|push|track|dolly/i.test(
  ...
  const cameraEnd: PrevisVector3 = moving
    ? [
        cameraStart[0] + (cameraTarget[0] - cameraStart[0]) * 0.18,
        cameraStart[1] + (cameraTarget[1] - cameraStart[1]) * 0.18,
        cameraStart[2] + (cameraTarget[2] - cameraStart[2]) * 0.08,
      ]
    : [...cameraStart];
  const sceneWorld = director.sceneWorld;
  ...
  return {
    ...
    output,
    scene: {
      ...(director.sceneWorldSourceAssetId ? { worldAssetId: director.sceneWorldSourceAssetId } : {}),
      ...(sceneWorld?.colliderMeshUrl ? { colliderMeshUrl: sceneWorld.colliderMeshUrl } : {}),
      ...(sceneWorld?.panoramaUrl ? { panoramaUrl: sceneWorld.panoramaUrl } : {}),
      backgroundColor: director.scene.backgroundColor,
    },
    actors: director.objects
      .filter((object) => object.kind === "character")
      .map((object, index) => {
        const position = toBlender(object.transform.position);
        const end: PrevisVector3 = moving
          ? [position[0] + 0.45 + index * 0.08, position[1], position[2]]
          : [...position];
        return {
          id: object.id,
          name: object.name,
          ...(sourceId(object.id) ? { sourceAssetId: sourceId(object.id) } : {}),
          scale: object.transform.scale,
          keyframes: [
            {
              frame: 1,
              position,
              rotationEuler: object.transform.rotation,
    ...
    props: director.objects
      .filter((object) => object.kind === "prop")
      .map((object) => ({
        id: object.id,
        name: object.name,
        ...(sourceId(object.id) ? { sourceAssetId: sourceId(object.id) } : {}),
        position: toBlender(object.transform.position),
        rotationEuler: object.transform.rotation,
        scale: object.transform.scale,
      })),
    camera: {
      lensMm: Math.max(
        10,
        Math.min(300, 36 / (2 * Math.tan((camera.fov * Math.PI) / 360))),
      ),
      keyframes: [
        { frame: 1, position: cameraStart, target: cameraTarget },
        { frame: frameCount, position: cameraEnd, target: cameraTarget },
      ],
    },
  };
}
