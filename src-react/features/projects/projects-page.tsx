...
import { AlertCircle, BookOpen, FolderOpen, Pencil, Plus, RefreshCw, Target, Trash2 } from "lucide-react";
    ...
    <section className="mx-auto max-w-7xl px-6 py-8 lg:px-10 lg:py-10">
      <header className="flex flex-wrap items-start justify-between gap-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Button type="button" variant="ghost" className="gap-2 border border-border" disabled={!api} onClick={() => openEditor(null)}><Plus aria-hidden="true" size={17} />新建项目</Button>
          <Button type="button" className="gap-2" disabled={!api} onClick={() => setGoalOpen(true)}><Target aria-hidden="true" size={17} />从目标开始</Button>
        </div>
      </header>
      ...
      {loading ? (
        ...
        <div role="alert" className="mt-8 flex items-start gap-4 rounded-xl border border-zinc-900/60 bg-zinc-950/20 p-5 text-zinc-200">
          <AlertCircle aria-hidden="true" className="mt-0.5 shrink-0" size={20} />
          <div><p className="font-medium">{error}</p><Button type="button" variant="ghost" className="mt-4 gap-2 border border-zinc-900/60" onClick={() => void refresh()}><RefreshCw aria-hidden="true" size={15} />重新加载</Button></div>
        </div>
      ) : projects.length === 0 ? (
        <div className="mt-8 grid min-h-72 place-items-center rounded-xl border border-dashed border-border bg-white/[0.02] px-6 text-center">
          <div><FolderOpen aria-hidden="true" className="mx-auto mb-4 text-slate-600" size={32} /><p className="text-lg font-medium">还没有项目</p><p className="mt-2 text-sm text-slate-500">新建项目后，从原文、资产和分镜开始生产。</p><Button type="button" className="mt-5 gap-2" disabled={!api} onClick={() => setGoalOpen(true)}><Target aria-hidden="true" size={16} />从目标开始新制作</Button></div>
        </div>
      ) : (
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {projects.map((project) => {
            ...
            return (
              <article key={project.id} className="flex min-h-56 flex-col rounded-xl border border-border bg-[#131313] transition-colors hover:border-zinc-500/50">
                <a href={projectHref(project)} onClick={(event) => void openProject(event, project)} aria-label={`打开项目 ${project.name}`} className="flex flex-1 flex-col p-5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
                  <div className="flex items-start justify-between gap-3"><h2 className="text-lg font-semibold tracking-tight">{project.name}</h2><span className="shrink-0 rounded-full border border-zinc-900/70 bg-zinc-950/40 px-2.5 py-1 text-xs text-zinc-300">{projectTypeLabel(project.projectType)}</span></div>
      ...
      {api && goalOpen ? (
        <ProjectGoalDialog
          ...
          onOpenDetailedConfig={() => { setGoalOpen(false); openEditor(null); }}
        />
      ) : null}
