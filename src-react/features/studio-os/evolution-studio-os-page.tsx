...
import type { EvolutionStudioOsApi } from "./evolution-studio-os-api";

export interface EvolutionStudioOsPageProps {
  api: EvolutionStudioOsApi;
}

/** Global governed professional-creation evaluation console. */
export function EvolutionStudioOsPage({ api }: EvolutionStudioOsPageProps) {
  ...
  return (
    <div className="space-y-8">
      <header className="rounded-xl border border-border bg-gradient-to-br from-[#0d1322] to-[#0a0d13] p-6">
        ...
        <button type="button" onClick={bumpReload} className="mt-4 inline-flex items-center gap-1.5 rounded-lg border border-border bg-white/[0.03] px-3 py-2 text-xs text-slate-300 transition-colors hover:bg-white/5">
