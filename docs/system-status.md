...
| Hodor API | 业务合同、PostgreSQL 持久化、智能体协调、文件和素材回写 |
...
| `/projects/$projectId/studio-os` | 项目组总览、故事室/导演室上下文、递归生产图、采用影响、租约、验证失败和演化证据 | `/studio-os-vnext/groups/:groupId/*` |
...
- Studio OS 控制室以 `groupId` 做项目隔离；采用和回滚动作携带确定性幂等键，完成后重新读取 Hodor 快照。缺失的 replay、shadow、canary 或 rollback 事件显示为未报告。
