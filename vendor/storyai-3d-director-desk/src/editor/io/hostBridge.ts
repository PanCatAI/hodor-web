...
export interface DirectorDeskEmbeddedHostCallbacks {
  onCapture?: (capture: { dataUrl: string; fileName: string }) => void | Promise<void>;
  onClose?: () => void;
}
...
let embeddedHostCallbacks: DirectorDeskEmbeddedHostCallbacks | null = null;
...
export function postDirectorDeskCapturesToHost(
  ...
  normalizedCaptures.forEach((capture) => {
    void embeddedHostCallbacks?.onCapture?.(capture);
  });
...
export function setDirectorDeskEmbeddedHostCallbacks(callbacks: DirectorDeskEmbeddedHostCallbacks) {
  embeddedHostCallbacks = callbacks;

  return () => {
    if (embeddedHostCallbacks === callbacks) {
      embeddedHostCallbacks = null;
    }
  };
}

export function notifyDirectorDeskClose() {
  embeddedHostCallbacks?.onClose?.();
